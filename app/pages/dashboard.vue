...
// ============================================================================
const stats = computed(() => dashboardStore.data?.stats ?? {
    totalActiveLoans: 0,
    pendingApprovals: 0,
...
const disbursementVsCollectionData = computed(() => {
    const months = dashboardStore.data?.monthlyDisbursements.map(d => d.month) ?? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
    const disbursements = dashboardStore.data?.monthlyDisbursements.map(d => d.amount) ?? [0, 0, 0, 0, 0, 0]
    const collections = dashboardStore.data?.monthlyCollections.map(d => d.amount) ?? [0, 0, 0, 0, 0, 0]
...
const loansByTypeData = computed(() => {
    const types = dashboardStore.data?.loansByType.map(l => l.type) ?? ['Salary', 'Emergency', 'Calamity', 'Equipment', 'Cash Advance']
    const counts = dashboardStore.data?.loansByType.map(l => l.count) ?? [0, 0, 0, 0, 0]
...
const loansByStatusData = computed(() => {
    const statuses = dashboardStore.data?.loansByStatus.map(l => l.status) ?? ['Active', 'Pending', 'Completed', 'Rejected', 'Defaulted']
    const counts = dashboardStore.data?.loansByStatus.map(l => l.count) ?? [0, 0, 0, 0, 0]
